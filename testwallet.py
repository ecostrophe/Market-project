...
class Vendeur:
    def __init__(self, nom):
        self.nom = nom
        self.wallet = Wallet(self.nom)
        self.wallet.add_money(500000)
        self.capital = self.wallet.get_balance()
        self.inventaire = []
        self.satisfaction=1
        self.impots = 0
    ...
    def payer_frais(self):#chaque 29
        ...
        self.wallet.pay((loyer+frais+self.impots))
        self.capital = self.wallet.get_balance()
        self.impots = 0
...
class Acheteur:
    def __init__(self, nom, budget):
        self.nom = nom
        self.wallet = Wallet(self.nom)
        self.wallet.add_money(budget)
        self.budget = self.wallet.get_balance()
        self.panier = []
    ...
    def demarrer_simulation(self, jours):
        for jour in range(1,jours):
            for acheteur in self.acheteurs:
                if jour%30==0:
                    acheteur.wallet.add_money(salaire)
                for vendeur in self.vendeurs:
                    ...
                    for produit in vendeur.inventaire:
                        ...
                        decision,round(offre-produit.prix,1),vendeur.get_inventaire()))
...
marche.demarrer_simulation(200)
